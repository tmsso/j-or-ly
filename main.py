...
def main():
    ...
    while input_word != "":
        if new_word:
            ...
            new_word = False
        input_word = input(f"Milyen szó rakható ki a következő betűkből: {random_letters}? ").strip()
        if input_word == "": break
        if len(input_word) == 1: 
            print(f"A teljes szó a következő volt:{random_word}")
            new_word = True
            continue
        if input_word not in word_list:
            print(f"Nem ismerek ilyen szót: {input_word}")
        elif can_form_word(input_word, random_word) and input_word in correct_guesses:
            print(f'Ezért a szóért már kaptál pontot. Pontszámod továbbra is {total_score}.')
        elif can_form_word(input_word, random_word):
            correct_guesses.append(input_word)
            current_score = len(input_word) ** 2
            total_score += current_score
            print(f"Helyes! {current_score} pont, összesen eddig {total_score}.")
        else:
            print(f"Ezekből a betűkből nem rakható ki {input_word}.")
